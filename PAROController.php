...
function getdetails(){
	...
	$command = "select *, (select office.officename from office where office.idoffice=dcrform.municipal) as municipalname, (select office.officename from office where office.idoffice=dcrform.provincial) as provincialname, (select office.officename from office where office.idoffice=dcrform.regional) as regionalname from trackform left join dcrform on trackform.iddcrform = dcrform.iddcrform where trackform.status='PARO' and trackform.officecode='$officecode' and trackform.enddate is null";
...
function completed($userid){
	...
	//if (empty($details)) {$details = "COMPLETED";}
