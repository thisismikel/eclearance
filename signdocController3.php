...
if (!empty($_REQUEST['signercd'])){$signercd=$_REQUEST['signercd'];} else {$signercd = 1;}
if (empty($_REQUEST['doctype'])){$doctype="clearance";} else {$doctype=$_REQUEST['doctype'];}
if (stristr(PHP_OS, 'WIN')) {
	if ($doctype=="clearance")
		{$finalname = realpath('')."\\landclearance\\".$filename;} else 
		{$finalname = realpath('')."\\areceipt\\".$filename;}
} else {
	if ($doctype=="clearance")
		{$finalname = realpath('').'/landclearance/'.$filename;} else 
		{$finalname = realpath('').'/areceipt/'.$filename;}
...
$iddcrform = $_REQUEST['iddcrform'];
		...
		for ($i = 1; $i <= $pageCount; $i++) {
			...
			if ($pageno == $i){
				...
				if ($signercd==2){
					...
					$bx = $x-42;
					$pdf->write2DBarcode('https://darxi.net/verifylandclearance.php?iddcrform='.$iddcrform, 'QRCODE,M', $bx, $y, 40, 40, $style, 'N');
				} else{	if ($x>50){$x = $x+65;}}
				$pdf->SetXY($x, $y);
		...
		if ($signercd == 2){ $signedfn = '2_'.$filename;} else {$signedfn = 'Signed_'.$filename;}
		if (stristr(PHP_OS, 'WIN')) {
			if ($doctype=="clearance")
				{$fullpath  = realpath('')."\\landclearance\\".$signedfn;} else 
				{$fullpath  = realpath('')."\\areceipt\\".$signedfn;}
		} else {
			if ($doctype=="clearance")
				{$fullpath = realpath('')."/landclearance/".$signedfn;} else 
				{$fullpath = realpath('')."/areceipt/".$signedfn;}
		...
		if (empty($signer)){
			if ($doctype=="clearance")
			{if ($signercd == 2){$command ="CALL updatelandclearance2($iddcrform, '$signedfn', $userid)";} else {$command ="CALL updatelandclearance($iddcrform, '$signedfn', $userid)";}} else 
			{$command ="CALL updateacknowledgementreceipt($iddcrform, '$signedfn', $userid)";}
