...
$tagvs = array(
 ...
 $pdf->province = "DAVAO CITY PROVINCIAL OFFICE";
 $pdf->setHtmlVSpace($tagvs);
	...
	if ($lsave == "0") {
		$pdf->Output('MARPO_Certificate.pdf', 'I');
	}else {
		$iddcrform = $_REQUEST['iddcrform'];
		$stamp = getdate();
		$fname = "Marpo".$iddcrform."_".$stamp['year'].$stamp['mon'].$stamp['mday'].$stamp['hours'].$stamp['minutes'].$stamp['seconds'].".pdf";

		//$fullpath = realpath('')."\\marpo\\".$fname;
		/////
		...
		$command ="CALL insertmarpocert($iddcrform, '$fname', $userid)";
		$row = getrow($command);
