...
if (stristr(PHP_OS, 'WIN')) {
		...
		$finalname = realpath('')."\\ireport\\".$filename;
} else {
...
$iddcrform = $_REQUEST['iddcrform'];
		...
		for ($i = 1; $i <= $pageCount; $i++) {
			...
			if ($pageno == $i){
				...
				if ($signercd==2){
					...
					$bx = $x-42;
					$pdf->write2DBarcode('https://darxi.net/verifyireport.php?iddcrform='.$iddcrform, 'QRCODE,M', $bx, $y, 40, 40, $style, 'N');
				} else{	if ($x>50){$x = $x+65;}}
				$pdf->SetXY($x, $y);
		...
		if (stristr(PHP_OS, 'WIN')) {
			$fullpath  = realpath('')."\\ireport\\".$signedfn;
		} else {
			$fullpath = realpath('')."/ireport/".$signedfn;
		}
		...
		$iddcrform = $_REQUEST['iddcrform'];
		$command ="CALL updatecarpocert($iddcrform, '$signedfn', $userid)";
